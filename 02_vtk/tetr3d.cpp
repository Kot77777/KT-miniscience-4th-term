...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double vz = 0;
            if(i > nodesCoords.size() / 3 - 100 && i < nodesCoords.size() / 3)
                vz = sin(pointX);

            nodes[i] = CalcNode(pointX, pointY, pointZ, smth, 0.0, 0.0, vz);
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "lion-step-" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    gmsh::initialize();
    gmsh::model::add("lion_stl");

    // Считаем STL
    try {
        gmsh::merge("../lion_stl.stl");
